...
def _make_name_record(text: str, lat: float, lon: float,
    ...
    type_code vocabulary (plan 01; callers must handle ``None``).

    ``kind`` is one of "road", "background", "place" and selects the
    level-0 on-disc encoding, per Ch.7.4 and a census of R's real level-0
    name records (plan 01; see also kiwiw/synth.py's "Names" section docstring
    ...
    (plan 01), `_handle_node` always passes 0x134
...
class _GeomHandler:
    ...
    def _handle_node(self, n) -> None:
        ...
        # (plan 01) traced Perth's level-2 spotcheck
