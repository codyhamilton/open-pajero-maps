...
| node +4 u16 y | Same encoding for y. y grows toward the south in `xy_to_latlon` (lat = lat_hi - y/range x span). | verified | Encoding round-trip as above. Orientation is unverified against a known point (`docs/phases/01-format-analysis.md` says unconfirmed). Encoding `verified`, orientation `assumed`. | `parser/kiwiw/coordconv.py` |
