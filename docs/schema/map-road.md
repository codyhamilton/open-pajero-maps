...
| 2 u16 n_intersections | Total intersection nodes (3+ roads), 0xFFFF = invalid. R carries 0xFFFF on the sampled parcel; G writes 0. | observed | Sampled Melbourne L0 parcel (748 links): 65535. Notebook `docs/archive/01-format-analysis.md` (uncertainty list) records the same and calls it a sentinel. Spec says 0xFFFF = invalid, consistent. Not censused across the disc. | `parser/kiwiw/road.py`, `parser/kiwiw/synth.py` |
...
| bit 8 toll_flag | Toll (charged) multilink. R: 1,147 of L0, 474 at L2, 346 at L4, 244 at L6, 111 at L8. | verified | Census counts in `map.json`; spec 8-1. OSM source rule (`toll=yes`) is assumed by design (`docs/design/map-layer-parity-remediation.md` F9), not measured. | `parser/kiwiw/road.py` |
...
| node +0 u16 bits 14:13 | Code name `planned` (0..3). Spec: one-way code (00 none, 01 one-way forward, 10 one-way backward, 11 two-way prohibited). R sampled: value 0 on 1,235 nodes, 1 on 263, 2 on 649, never 3 (2,147 nodes, Melbourne L0). Populations are inconsistent with a "building-planned" flag and fit one-way streets. | unknown | Sampled parcel (this document). Code name and `map-layer-parity-remediation.md` F9 ("oneway is 0 on all nodes: one-way is not in the map layer") conflict with the spec reading. No test distinguishes. Winner: undecided; do not populate from OSM until decided. | `parser/kiwiw/road.py`, `parser/kiwiw/synth.py` |
...
| node +4 u16 y | Same encoding for y. y grows toward the south in `xy_to_latlon` (lat = lat_hi - y/range x span). | verified | Encoding round-trip as above. Orientation is unverified against a known point (`docs/archive/01-format-analysis.md` says unconfirmed). Encoding `verified`, orientation `assumed`. | `parser/kiwiw/coordconv.py` |
...
`parser/refdata/vocab/README.md` and `parser/kiwiw/roadtypes.py` state that the spec has no code
...
| 2 | Highway class > 91 km/h (national road). R: 848,884 at L0, 58,909 at L2, >= 95% of L4-L8. The dominant coarse-level class. Parity design calls it R's "main arterial class". | spec-only | Spec ch.32.2; `map.json`; `docs/design/map-layer-parity-remediation.md` F4 (name-matched sampling of Brisbane and Longreach). | `parser/refdata/vocab/road_type.json` |
...
| 7 | Private road. R: 38,493 at L0, 6 at L2, 5/3/2 at L4/L6/L8. `vocab/README.md` maps OSM primary to 7 (by rarity ranking). Spec label conflicts with that mapping. | spec-only | Spec ch.32.2; `map.json`; `vocab/README.md`. Conflict recorded: the spec label wins as the format definition, but R's use of 7 at coarse levels (roads kept when nearly everything else is dropped) suggests R does not use the overseas labels literally. | `parser/refdata/vocab/road_type.json` |
| 8 | Walkway. R: 71,032 at L0. `vocab/README.md` maps OSM secondary to 8. | spec-only | Spec ch.32.2; `map.json`. Same caveat as code 7. | `parser/refdata/vocab/road_type.json` |
...
| Display class order vs spec ch.32.1 | R's class values are not the spec's overseas labels (spec 12 = walkway, R 12 = type 0 = freeway; spec 3 = freeway, R 3 = type 8). The ordering is not a prominence rank, as `vocab/README.md` observed. | observed | Compare `map.json` pairing above with spec ch.32.1. Meaning of R's class values comes from metadata not read here. | `parser/refdata/vocab/display_class.json` |
...
| Field | Meaning | Status | Evidence | Code |
|---|---|---|---|---|
| `road_type.json` ranges [0,0], [2,8], [10,12] | L0: motorway 0, trunk 10, primary 7, secondary 8, tertiary 3, unclassified/road 5, residential/living_street 6, service/busway 9, track 2. L2-L8: motorway 0, trunk 10, primary 7, everything else 2. L10/L12: none. | assumed | Derived by rarity ranking (`parser/refdata/vocab/README.md`); the spec labels above contradict trunk -> 10 and suggest primary/secondary -> 2 and 3. Parity design F4 targets: trunk/primary -> 2 (class 10), secondary/tertiary -> 2 or 3 by level, residential -> 6, track -> 9, motorway 0. Only the value-set membership is tested (`parser/tests/test_vocab.py`). | `parser/kiwiw/vocab.py` |
