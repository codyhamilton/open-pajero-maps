...
| ISO volume size | 1,166,832 sectors of 2048 B (= image size 2,389,671,936 B) | observed | PVD of R image; `docs/archive/00-inventory.md`. | - |
| Directory tree | Root holds 21 files plus dirs `COVERAGE/` (1 file), `DN/` (1 file), `IDX/` (99 files, dir extent 3 sectors). Names are ISO level-1 style (`NAME.EXT;1`) | observed | Walk of R image root; `docs/archive/00-inventory.md`. | - |
| Root file set | `ALLDATA.KWI`, `LOADING.KWI`, `METADATA.KWI`, `SPEC.KWI`, `COUNTRY.KWI`, `VERSION.TXT`, `COVERAGE.BIN`, `INDEXDAT.KWI`, `HWMAP.KWI`, `DICVCE56.KWI`, `GRA256D.KWI`, `KGRA256.KWI`, `PCT256D.KWI`, `KPCT256.KWI`, `PCT2DAT.KWI`, `KPCT2DT.KWI`, `PCT2MNG.KWI`, `KGRPDAT.KWI`, `VAR256D.KWI`; plus `DN/CLUSTER.DAT`, `COVERAGE/AUC.BMP` | observed | Listing of R; `docs/archive/00-inventory.md` (sizes). Spec ch.2.1 names only four basic files (ALLDATA, METADATA, SPEC, LOADING); the rest are vendor additions or defined in other chapters. R wins. | - |
...
| Field | Meaning | Status | Evidence | Code |
|---|---|---|---|---|
| `GRA256D.KWI`, `PCT256D.KWI`, `PCT2DAT.KWI` + `K*` partners, `VAR256D.KWI`, `KGRPDAT.KWI` | Image data frames (spec ch.33) and their key tables, tied together by `PCT2MNG.KWI` group ids | observed | Filename/size fit and the manifest in `docs/archive/01-format-analysis.md`; pixel/palette content not decoded. | `parser/kiwiw/misc.py` |
| `DICVCE56.KWI` | Voice data frame (spec ch.34) | observed | Size and disc-stamp header only, `docs/archive/01-format-analysis.md`. | `parser/kiwiw/misc.py` |
| `HWMAP.KWI`, `INDEXDAT.KWI` | Named by MHT records 18 and 2 (see MHT); internals undecoded | unknown | Size/header sniff only, `docs/archive/01-format-analysis.md`; owner WP4 in `docs/design/target-disc.md`. | - |
...
| 27232..~26.6 MB block records | One Parcel Management Record per block, packed contiguously with no gaps in level order 12, 10, 8, 6, 4, 2, 0 (each level's range starts where the previous ended: 27264, 27456, 28896, 47680, 171424, 1641312; level 0 ends at 26,596,192) | observed | Measured min/max block offset per level from the BMT of R (7 levels, 2307 entries). Per-block contiguity also in `docs/archive/02-roundtrip.md` (level 8 sample, "MEDIUM"). Not asserted by a test. | `parser/kiwiw/alldata_writer.py` |
| ~26.6 MB..: Map Frames (leaves) | Leaf frames in increasing offset order with variable gaps (1.6 KB to ~480 KB in the sampled region), other-level content interleaved | observed | `docs/archive/02-roundtrip.md` ("WEAK / not established"). Placement rule of the original tool not derived. | - |
...
| 456 (16 B) Data coverage: lower-left PID, upper-right PID | R: S50, E90 to N35.3333, W142 (box crosses +/-180); exponent bytes 0 | verified | Round trip; kiwiread output in `docs/archive/00-inventory.md`; agrees with PDMDH coverage (`docs/design/target-disc.md` cross-file consistency). Longitude is expressed east-positive with the right edge wrapped negative. | `parser/kiwiw/volume.py` |
...
| Field | Meaning | Status | Evidence | Code |
|---|---|---|---|---|
| Coverage box and wrap | Coverage is PDMDH lower-left/upper-right; longitude span is `right - left`, plus 360 when negative (R: 90 E to 142 W) | verified | `parser/tests/test_mesh.py` (Melbourne cell matches patched kiwiread, `bs 22 block 23 parcel 542`); antimeridian fix documented in `docs/archive/00-inventory.md`. | `parser/kiwiw/mesh.py` |
