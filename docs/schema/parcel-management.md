...
| Cell to tiers | `px = ix % npc_lng`, `blx = (ix // npc_lng) % nbl_lng`, `bsx = ix // (npc_lng*nbl_lng)`; the same for latitude. Then `bset_flat = bsy*nbs_lng + bsx`, `block_flat = bly*nbl_lng + blx`, `slot = py*npc_lng + px` | verified | `parser/tests/test_mesh.py` (Perth, Melbourne, Sydney resolve to correct places); `docs/phases/01-format-analysis.md` records 799/800 street-name agreement against an independent index oracle (analysis, not a repo test) | `parser/kiwiw/mesh.py`, `parser/kiwiw/alldata_writer.py` |
...
| Block buffer placement | Block buffers of one block set are contiguous, each starting where the previous ended, in BMT order (observed on one level-8 region, 6 blocks) | observed | `docs/phases/02-roundtrip.md` "Whole-file ALLDATA.KWI assembly"; not rechecked disc-wide | `parser/kiwiw/alldata_writer.py` |
