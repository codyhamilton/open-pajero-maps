...
2026-09-23 (user) — grounded phase gates") as measured by the committed, re-run tools.
**Phase 2 did not close on this re-run** (`GATE-2.md`): criterion 1 (coordinate range)
and criterion 3 (divided quadrant containment) pass; criterion 2 (cross-parcel
continuity) fails; criterion 4 (boundary-node mirror) has a real L0_sparse failure.
The rows below record what each criterion actually measured, not a gate pass.
...
| **Criterion 1 — coordinate max vs class range** | No parcel coordinate exceeds its class range. `exceeds_max` (a different count from `exceptions`, which is a non-max observed peak, not a violation) is 0 in every one of 28 (level, class, division) classes, over the full-disc denominator of 452,199 content-bearing parcels. Re-run 2026-09-23 against R: `ranges` and `class_rule` structurally identical to the committed profile. **PASS** | observed | `coord_scale.json` `ranges` (28 classes, `exceeds_max`); `EVIDENCE-2-12.json` `criterion_1` | `parser/tools/coord_scale_census.py` |
| **Criterion 2 — cross-parcel continuity** | A road link crossing a parcel boundary is stored independently on both sides; decoded under the assumed (range, frame), the two copies of a shared endpoint must land on the same place (`EDGE_TOL_RAW=4`, `PAIR_TOL_RAW=16`, pass = separation < 1.0 raw unit, pre-stated). Alternatives discriminated against: `half` range, `double` range, `decoder_range` (32768), and for divided parcels `own_leaf_bbox_own_range`. Measured (`EVIDENCE-2-10.json`, re-run byte-identical, sha256 `8041db0f66...`): median separation 0.0 m in every class (L0_urban 405 pairs, L0_sparse 3068, L2 550, L4 416, L6 383, L8 34, divided_pardiv1 984) against alternative medians of 1,182-294,864 m. But `over_threshold_total`=407 across classes, with L0_sparse's 328-of-3068 over-threshold pairs **not fully enumerated** (200-example cap short of 328) — the criterion demands zero violations or a residual explained record by record, and this residual is neither. **FAIL** | observed | `EVIDENCE-2-10.json` `criterion_2_continuity` (`over_threshold_total`, per-class `over_threshold`/`over_threshold_fully_enumerated`); `EVIDENCE-2-12.json` `criterion_2` | `parser/tools/continuity_census.py` |
| **Criterion 3 — divided-parcel quadrant containment** | Spec 7.2.2.1.1.2 (2) and (3): a divided parcel's normalized coordinate uses the original basic-parcel frame with each sub-parcel's relative position set to 0 — the 2x2 sub-parcel model. Every shape point of sub-parcel k (0=SW,1=SE,2=NW,3=NE) must lie inside quadrant k of the parent leaf's 4096 frame (`parent_local` reading). Measured: 168 sub-parcels (52 content-bearing), 1,037,716 shape points, 0 violations under `parent_local`. The `sub_local` reading (798,575 violations) is reported as the rejected alternative, not a competing claim. **PASS** | observed | `EVIDENCE-2-10.json` `criterion_3_quadrant`; `EVIDENCE-2-12.json` `criterion_3`; spec 7.2.2.1.1.2 (2), (3) | `parser/tools/continuity_census.py` |
| **Criterion 4 — boundary-node mirror** | A link end-node whose raw coordinate is exactly 0 or exactly the class range is a genuine boundary crossing: the adjacent parcel (found via same-block, cross-block or cross-blockset neighbour lookup, 2-09) holds an end-node at the mirrored coordinate (crossed axis = range - value, other axis unchanged), matched at exact integer equality. Denominator excludes nodes at the extract's outer edge (`outside_coverage`) and empty neighbour slots (`empty_slot`) rather than failing them; `scale_mismatch` (a neighbour that resolves to a divided leaf, a different coordinate scale) is excluded the same way, which means the claim's reach stops at undivided-to-undivided crossings — it says nothing about a boundary against a divided neighbour. Per class (denominator/matched/violations/scale_mismatch): L0_urban 408/408/0/114 pass; L2 4067/4061/6/111 pass with residual; L4 825/825/0/45 pass; L6 746/746/0/104 pass; L8 66/66/0/23 pass; **L0_sparse 11536/2884/8652/0 — 8652 of 11536 (75%) fail**, most same-block, examples show the neighbour resolving far from the expected mirrored coordinate. This is a real gate failure for L0_sparse, not fixable within a fixed-constant scope: L0_sparse leaves may not tile edge-to-edge the way other classes do, or need a different mirror rule under the tile frame. **FAIL (L0_sparse); PASS/PASS WITH RESIDUAL elsewhere** | observed | `EVIDENCE-2-11.json` `classes`; `EVIDENCE-2-12.json` `criterion_4`; spec 7.2.2.1.1.3 (on-boundary node flag) | `parser/tools/boundary_mirror_census.py` |
