...
| POISR SRMX POI | BFRL NFRL ARCD FGFZ STFG(2) CTGY(UW) ZIPN RLXY NXKD NXFN NXST NXCT KYCH NAME RPLV RPAT RPCN RPLN RPST RPSO; one record per word-suffix of a name; POISR202 10,969 records, STFG varies (fd07, fd06, fc07 ...) | observed | R walk of POISR202; docstring of `iter_matching_records` claims POISR201 populations decode byte-identically but no test covers it. `docs/ARCHITECTURE.md` says POISR decoder has bugs; docstring untested, so observed only. First test: compare POISR201 and POISR202 record counts and field values; identify which fields trigger decoder bugs in WP3 work | `parser/kiwiw/search_frame.py` |
| CTGY | Multiples of 128; includes vendor codes absent from the spec (~38% of QLD sample); category-name table undecoded | observed | sample of R. First test: enumerate all CTGY codes in POISR201-207 and POIDT001-013; separate spec-defined codes from vendor codes; map to OSM amenity/shop vocabulary (WP3 OSM source mapping) | - |
...
| ITSSR | SRBT (BFRL FDUH NFRL STID NCST STFG ARCD) + nested SRAL (BFRL NFRL STFG FGFZ KYCH LGNO NAME POIO POIC); ITSSR202 2,803 SRBT, STFG 0 | observed | R dump. Bodies undecoded (WP4). First test: parse ITSSR202 header and count SRBT/SRAL records; hex-dump one SRBT and one SRAL record, then verify round-trip structure against field offsets | - |
| FWYSR | SRFW (FGFZ KYCH NAME NXKD NXFN NXST) + nested SRRP (SFTO SFBO SEFG SELN DCSF BFRL NFRL KYCH NAME RLXY NXKD NXFN NXST NXSZ); FWYSR204 23 SRFW | observed | R dump. Bodies undecoded (WP4); absent on R for NT (202) and TAS (207) (FINDINGS section 4). First test: verify FWYSR exists for states 201/203/204/205/206 and absent for 202/207; parse one SRFW + SRRP record and verify byte alignment | - |
| AGMSR/ARGSR/EMGSR | DFSR SRAG genre hierarchy (JPTB SFTO SFBO SELN DCSF BFRL NFRL KYCH KBTP STFG NXKD NXFN NXST DSRA CTGY CTG2 VOID NAME TPNM); AGMSR nested SRME (SMEN SMEL NAOF NACT) then SRAR (CTGY RLXY POIO), 519,215 records | observed | R dump. Bodies undecoded (WP4). First test: parse AGMSR header and count SRAG/SRME/SRAR records; hex-dump one example of each and verify byte structure matches field offsets from FINDINGS section 4 (WP4) | - |
| DB0/JG0/LR0/MB0/MZ0/ND0/NF0/NS0/VL0 suffix set | Unexplained; headers differ only in record size | unknown | R headers | - |
| ARSNC/ARSSR/EM2SR/EM3SR | DFSM SRME (SMEN u32, SMEL u8 mesh, NXKD/NXFN HB, NXST) then SRAR (CTGY RLXY POIG POIO) or SRNC (ARCD RLXY POIG POIO) | observed | R dump. Bodies undecoded (WP4). First test: parse ARSNC202 header and count SRME/SRAR records; verify record size matches DFSM 0xca-0xce from index-idx.md line 34; hex-dump one SRME and SRAR pair | - |
| POIAS | DSRC SRBC (STID ARCD NXKD NXFN NXOF; mdef adds CTGY VRUW, POIO) | observed | R dump | - |
| ZONE*SRC | SRZN (SZCD DCFN NXKD NXFN NXST NXFM CH*16), SELN=7, category data at 508, 26-byte records | observed | R dump. Bodies undecoded (WP4). First test: parse ZONEZSRC header and count SRZN records; verify each is 26 bytes; hex-dump one record and map fields against declared structure; confirm category data at offset 508 | - |
| ZSEL* | SRSZ (VDID DESL SFTO SFBO SELN DCSF BFRL NFRL SZCD VDID NAME TPNM DFKB SZDI), keyboard KBC1; state names multi-language | observed | R dump. Bodies undecoded (WP4); emergency and zone data have no practical OSM source (FINDINGS section 4). First test: parse ZSEL201 header and extract state names; verify multi-language entries; confirm keyboard KBC1 structure | - |
| FMCDT001 | DFM2 SRBT keyed by ARCD; catdef SELN DCSF ARCD NXKD NXFN NXST, mdef BFRL NFRL NAME; 11,997 records | observed | R dump; semantics unknown (WP4). First test: parse FMCDT001 header and count SRBT records; verify byte structure against DFM2 format and field offsets; compare R vs. G to identify regenerate strategy | - |
| category_data name table | Category names for CTGY | unknown | undecoded. First test: locate category_data table in INDEXDAT.KWI (FINDINGS section 4 says category data at offset 508 in ZONE files); parse TLV entries and map each code to a name; verify coverage against CTGY codes from POISR/POIDT | - |
