...
|---|---|---|---|
| `ALLDATA.KWI` map layer (Ch.5–7: volume, MHT, PDMDH/LMR/BSMR/BMT, parcel mgmt, road/background/name frames) | generate | WP1 | **2026-09-09 full-Australia build (WP1 units 15/15b), `compare_disc.py` report:** container FAIL (1 unallowlisted PDMDH-blob-length diff, R=21,088 G=18,624 bytes, non-zero extra tail — cause not isolated); envelope FAIL (20 failures, levels 8/6/4/2/0: parcel_count and name_count outside the [0.5,2.0]x envelope — unit 14's `selection.json` calibration under/over-shoots on several counts it flagged as approximation risk; several background/name sub-frame maxes exceed R's per-level max); mfde FAIL (`nregion` never 1 — expected, see WP2 slot row below; mfde entry-count never exceeds 20 where R sometimes has 21–35, and entry-index 1 is always "absent" where R sometimes has content — **not** a declared WP2 slot, unresolved); spotcheck FAIL (Sydney level 0 missing all 3 expected road names; Melbourne level 0 missing 2 of 3; Perth level 2 missing its place name); vocab FAIL (name_type_code values 288/289/290/306/321/578 appear at levels where R's per-level census does not have them — likely background-type codes leaking into name records' type_code field beyond R's own per-level name vocabulary, and `bg_type.json`'s `railway=rail → 578` rule at level 0 in particular has no corresponding entry in R's level-0 name census). decode/pointers/mht29/shape all PASS. **Levels 10 and 12 spooled zero content** (root cause identified as a real bug, not fixed here: `selection.json` admits `natural=dune` at levels 10/12, but `vocab/bg_type.json` has no mapping for `natural=dune` at any level, so `_osm_tags_to_bg_type` returns `None` and every such way is silently dropped before `spool.add`; unit 14's own selection.json vs. unit 08's vocab/bg_type.json are inconsistent). Separately, `compare_disc.py`'s per-level checks only iterate the *generated* profile's levels, so R-vs-G level 10/12 discrepancies are not even reported as FAIL — a harness gap, not a passing result. Full report: `output/report.json`, `docs/plans/01-eval-harness-and-map-layer.md`'s "Build record (2026-09-09)". **2026-09-16 full-Australia rebuild #2 (WP1 units 25/25b, run after ad-hoc briefs 19-24 landed), `compare_disc.py` report:** container still FAIL, **byte-identical symptom to 2026-09-09** (1 unallowlisted PDMDH-blob-length diff, R=21,088 G=18,624 bytes, extra 2,464 bytes not all zero) — brief 19's `natural=bay` re-selection fix at levels 10/12 is confirmed working (both levels now spool/encode real content: level 10 has 4 parcels/26 backgrounds, level 12 has 2 parcels/26 backgrounds) but does **not** close the PDMDH-length gap at full scale; root cause of the remaining 2,464-byte shortfall not isolated, reported for a follow-up fixer. vocab flipped to PASS (brief 23's fix). mfde FAIL narrowed to 1 failure (from 5). spotcheck FAIL unchanged (same 3 rows). envelope still FAIL (23 failures; fresh per-level parcel_count/name_count generated/reference/ratio data for all seven levels recorded in `docs/plans/01-eval-harness-and-map-layer.md`'s "Build record (2026-09-16)", feeding unit 26's recalibration). decode/pointers/mht29/shape all PASS. Full report: `output/report.json`, `docs/plans/01-eval-harness-and-map-layer.md`'s "Build record (2026-09-16)". **2026-09-19 rebuild #4 (units 31/31b):** container, decode, pointers, mfde, mht29, shape, vocab PASS; envelope FAIL (5: L0 name_count 0.103x and L12 parcel_count 3 vs 1 are proposed declared deviations pending acceptance; L0 background, L8 road and L8 background sub-frame maxima are WP1 fixes); spotcheck FAIL (Perth/Adelaide L0, WP1 fix). Details: PLAN.md "Build record (2026-09-19, rebuild #4)". **2026-09-19 rebuild #5 (units 32b/32c):** spotcheck now PASS (14/14); envelope FAIL (3): L0 name_count 0.149x and L12 parcel_count 3 vs 1 are proposed declared deviations pending acceptance (as are L0 background 1.46% and L8 name 12.9% trim >1%); L8 road sub-frame max 121,080 > R 99,794 is a WP1 fix. Details: PLAN.md "Build record (2026-09-19, rebuild #5)". **2026-09-19 rebuild #6 (units 33b/33c):** envelope FAIL (2 only): L0 name_count 0.149x and L12 parcel_count 3 vs 1, both proposed declared deviations pending acceptance, as are >1% trims at L0 background (1.46%) and L8 road/name/background; all sub-frame kind maxima <= R (L8 road 99,786); spotcheck 14/14; no WP1-scope fix remains. Details and consolidated deviation list: PLAN.md "Build record (2026-09-19, rebuild #6)". |
...
- the old overview's 2026-08-24 test-access decision ("favour frequent
