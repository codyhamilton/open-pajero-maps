...
### Orchestrator restart (2026-09-25)
The first 3C orchestrator hit an API usage limit at about 11:53 and its workers died with it (no processes left; a stale waiter shell was killed). Resumed cold from the committed record by a fresh orchestrator (Opus 5.5). State found: 3C-05 committed as 664f7fd but unverified and unrecorded; 3C-01 left an uncommitted ~290-line diff inside its owned paths with no report (saved as `output/scratch-3C-01/handoff-uncommitted.diff` and handed to a fresh 3C-01 worker to review against the brief, not trusted); 3C-04 was dispatched but left no changes, so it is re-dispatched fresh.

### 3C-05 coord-scale-parallel — done with concerns (664f7fd, Sonnet)
**Built:** `compare_disc.py --workers N` (default 1, unchanged behaviour) passed through `harness/context.py`; `harness/checks/coord_scale.py` partitions `walk.iter_blocks`' blocks round-robin over a process pool, each worker folds its shard with the same per-parcel `_accumulate` the sequential path uses, and the parent merges shards in fixed order (`_merge`/`_finalize`), so verdict and counts are independent of worker count. `test_harness_coord_scale.py` +43 lines (1-vs-4-worker identity on Perth, injected out-of-range coordinate caught).
**Evidence (re-run by the orchestrator after the restart; the worker's report was lost):**
- pytest `test_harness_coord_scale.py` + `test_coord_scale_census.py`: 19 passed.
- `compare_disc --checks coord_scale --workers 12` on the 3-11 disc (`87a01b14…`, sha confirmed in the report): PASS, 0 of 1,461,347 parcels, 73 classes; every `details` key including all 73 `per_class` entries equal to `output/scratch-3-11/cs_G.json`. Wall **112.7 s** (`/usr/bin/time`, lock wait excluded; load average ~8 from concurrent 3C-01/3C-04 work), peak RSS of the parent 26 MB. Worker's own figure in its commit: 111.3 s. Was 783 s single-process. Budget ≤ 120 s: met.
