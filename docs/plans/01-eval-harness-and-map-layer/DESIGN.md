...
| 10 | ext slot 8 | mixed `absent`/`in_buffer` at every level (roughly 88-90% absent, 10-12% in-buffer) | L6-L12: byte-identical duplicate of the name sub-frame (mfde[2]) when a name frame is present; L0-L4: absent | WP1 (brief 30). R census: at L6/L8/L10/L12 idx10 is in_buffer iff name present, size equal, 100% of name-present leaves. L0-L4 residual variant (idx10 present without a name, differing size) rule still undecoded; absent = dominant class |
...
| mfde indices 4, 10 (rare in-buffer ext content) | idx 10: name-frame duplicate at L6-L12 (brief 30), absent at L0-L4; idx 4: `(0xFFFFFFFF, 0)` | Not clearly either WP's scope yet — undecoded sub-frame kind, format-analysis dead end per the brief's stated escape hatch ("if neither is possible ... goes to the user") | none yet |
